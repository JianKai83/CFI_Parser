...
class poseDetector():
    ...
    def cosin2goc(self, img, p1, p2, p3, draw=True):
        ...
        u = [x1-x2, y1 - y2]
        v = [x3-x2, y3 - y2]
        cosin = ((u[0] * v[0]) + (u[1]*v[1]))/(math.sqrt(u[0] * u[0] + u[1] * u[1]) * math.sqrt(v[0] * v[0] + v[1] * v[1]))
...
def main():
    cap = cv2.VideoCapture('./video/bench_press.mp4')
    pTime = 0
    ...
    while (cap.isOpened()):
        ...
        cv2.putText(img, str(int(fps)), (20, 20), cv2.FONT_HERSHEY_PLAIN, 3,
                    (255, 0, 0), 3)
